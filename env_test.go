...
func Test_environCredentials(t *testing.T) {
	fKey, err := os.CreateTemp("", "tmpfile-")
	...
	_, err = fKey.WriteString(key)
	require.NoError(t, err)

	fOauth2Key, err := os.CreateTemp("", "tmpfile-")
	require.NoError(t, err)
	t.Cleanup(func() {
		fOauth2Key.Close()
		os.Remove(fOauth2Key.Name())
	})
	oauth2Key := `{
		"token-endpoint": "https://localhost:123/token/exchange",
		"subject-credentials": {
			"type": "JWT",
			"alg": "RS256",
	...
	_, err = fOauth2Key.WriteString(oauth2Key)
	require.NoError(t, err)
	...
	for _, tt := range []struct {
		...
		},
		{
			...
			env: map[string]string{
				"YDB_SERVICE_ACCOUNT_KEY_FILE_CREDENTIALS": fKey.Name(),
			},
			credentials: func() credentials.Credentials {
				c, err := yc.NewClient(yc.WithServiceFile(fKey.Name()))
				require.NoError(t, err)
				return c
			}(),
		},
		...
		},
		{
			...
			env: map[string]string{
				...
				"YDB_SERVICE_ACCOUNT_KEY_FILE_CREDENTIALS": fKey.Name(),
				"YDB_SERVICE_ACCOUNT_KEY_CREDENTIALS":      key,
				"YDB_OAUTH2_KEY_FILE":                      fOauth2Key.Name(),
			},
			credentials: func() credentials.Credentials {
				c, err := yc.NewClient(yc.WithServiceFile(fKey.Name()))
				require.NoError(t, err)
		...
		{
			name: "oauth2 key file credentials only",
			env: map[string]string{
				"YDB_OAUTH2_KEY_FILE": fOauth2Key.Name(),
			},
			credentials: func() credentials.Credentials {
				c, err := credentials.NewOauth2TokenExchangeCredentialsFile(fOauth2Key.Name())
				require.NoError(t, err)
				return c
			}(),
		},
